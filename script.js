...
function initMain() {
    ...
    $("scrubber-track").addEventListener("click", scrubberClick);
    ...
    // Agents
    $("sim-agent-btn").addEventListener("click", simAgentJoin);
    ...
    if ($("sim-agent-btn")) {
        $("sim-agent-btn").addEventListener("click", () => {
            ...
            spawnAgent(tempId);
            setTimeout(() => {
                if (activeAgents[tempId]) {
                    // Simulating a random disconnect after 2 minutes for realism
                }
            }, 120000);
        });
    }
...
function setJoinCodes() {
    $("join-code-display").textContent = JOIN_CODE;
...
function initSocketAndWebRTC() {
    ...
    if (ROLE === 'broadcaster') {
        ...
        // Signalling handlers for Listener (if any specific needed beyond listener.html)
        socket.on('webrtc-offer', async ({ senderId, sdp }) => {
            addMsg("SYSTEM", `Incoming signal from Broadcaster...`, CYAN, true);
        });
    }
...
const activeAgents = {}; // socketId -> { rosterIndex, element }
    ...
    if (playing) {
        playInterval = setInterval(() => {
            if (video && video.src && !video.paused) {
                ...
                // Emit fallback frame at ~25fps for smooth local playback
...
function cycleSpeed() {
    ...
    if (playing && (!video || !video.src)) { clearInterval(playInterval); togglePlay(); togglePlay(); }
}
...
function updateTimecode() {
    ...
    const f = fmtTc(tc);
    $("scrubber-tc").textContent = f;
    $("tc-display").textContent = f;
    $("timecode-display").textContent = f;
}
...
function updateDiagDynamic() {
    document.querySelectorAll("[data-dynamic='sync']").forEach(el => {
        el.textContent = AGENTS.filter(a => a.status === "SYNCED").length + " / " + AGENTS.length;
    });
    document.querySelectorAll("[data-dynamic='uptime']").forEach(el => {
        el.textContent = fmtTc(Math.floor(Date.now() / 1000) % 3600);
    });
    const sb = $("sync-badge");
    if (sb) sb.textContent = AGENTS.filter(a => a.status === "SYNCED").length + "/" + AGENTS.length + " SYNCED";
}
...
function renderAgentsMain() {
    const tbody = $("agents-main-body");
    if (!tbody) return;
    tbody.innerHTML = "";
    AGENTS.forEach(a => {
        const tr = el("tr");
        const td1 = el("td");
        const nameWrap = el("span", "agent-name-cell");
        const dot = el("div", "led sm blink " + (a.status === "SYNCED" ? "green" : "amber"));
        nameWrap.style.color = a.color;
        nameWrap.append(dot, document.createTextNode(a.name));
        td1.appendChild(nameWrap);

        const td2 = el("td", "", a.ping + "ms"); td2.style.color = TEAL;

        const td3 = el("td");
        const bars = el("div", "signal-bars");
        const lit = a.ping < 100 ? 4 : a.ping < 200 ? 2 : 1;
        [3, 6, 10, 13].forEach((h, i) => {
            const b = el("div", "signal-bar" + (i < lit ? " lit" : ""));
            b.style.height = h + "px";
            bars.appendChild(b);
        });
        td3.appendChild(bars);

        const td4 = el("td", "", "+" + a.drift + "ms"); td4.style.color = a.drift < 200 ? GREEN : AMBER;

        const td5 = el("td");
        const sb = el("span", "status-badge " + (a.status === "SYNCED" ? "synced" : "drifting"), a.status);
        td5.appendChild(sb);
...
function simAgentJoin() {
    const used = AGENTS.map(a => a.name);
    const next = SIM_AGENT_NAMES.find(n => !used.includes(n));
    if (!next) return;
    AGENTS.push({ id: Date.now(), name: next, ping: Math.floor(Math.random() * 150 + 20), drift: Math.floor(Math.random() * 300 + 50), status: "SYNCED", color: CYAN });
    renderAgents();
    addMsg("SYSTEM", "▶ AGENT CONNECTED: " + next, GREEN, true);
    showToast("NEW AGENT CONNECTED", next + " joined the broadcast.");
}
...
function toggleCover() {
    ...
    const btn = $("alarm-btn");
    cover.classList.toggle("open", coverOpen);
    btn.classList.toggle("armed", coverOpen);
    btnArmed = coverOpen;
}
...
function updateDebugGrid() {
    ...
    const cells = [
        { t: "STUN SERVER", rows: [["Server", "stun.l.google.com", CYAN], ["Port", "19302", null], ["Status", "REACHABLE", GREEN], ["RTT", "12ms", GREEN], ["NAT Type", "FULL CONE", AMBER], ["Public IP", "203.0.113.7", CYAN]] },
        { t: "WebRTC / ICE", rows: [["ICE State", "CONNECTED", GREEN], ["Protocol", "UDP / SRTP", CYAN], ["DTLS", "CONNECTED", GREEN], ["Fingerprint", "sha-256:AF3B…", TEAL], ["Local", "192.168.1.5:54311", TEAL]] },
        { t: "HEALTH", rows: [["Avg RTT", "18ms", GREEN], ["Jitter", "4ms", CYAN], ["Pkt Loss", "0.4%", GREEN], ["Bandwidth", "2.4 MB/s", CYAN], ["Peers", peerList.length + "", GREEN]] },
        ...peerList.map(a => ({ t: "PEER — " + a.name, rows: [["Status", "ACTIVE", GREEN], ["ICE", "CONNECTED", GREEN]] }))
    ];
    cells.forEach(cell => {
        const div = el("div", "debug-cell");
        const title = el("div", "debug-cell-title", cell.t);
        div.appendChild(title);
        cell.rows.forEach(([k, v, c]) => {
            const row = el("div", "debug-row");
            const kEl = el("span", "", k);
            const vEl = el("span", "", v);
            if (c) vEl.style.color = c;
            row.append(kEl, vEl);
            div.appendChild(row);
        });
        g.appendChild(div);
...
function startRadar() {
    ...
    function draw() {
        ...
        window.radarBlips.forEach(b => {
            ...
            ctx.fillStyle = b.col.startsWith("var") ? CYAN : b.col; // Simple fallback for css vars in canvas
...
function playWarningSound() {
    try {
        ...
        osc2.frequency.value = 505; // slight detune

        lfo.type = 'square';
        lfo.frequency.value = 2; // 2Hz switching for siren effect

        lfo.connect(lfoGain);
        lfoGain.gain.value = 200; // frequency shift amount
        lfoGain.connect(osc1.frequency);
        lfoGain.connect(osc2.frequency);

        // Attack, sustain, release envelop
...
function loadMedia(src, name) {
    ...
    if (video) {
        ...
        video.onerror = () => {
            addMsg("SYSTEM", "SIGNAL FAILED — INVALID MEDIA", RED, true);
            showToast("ERROR", "Invalid media signal");
            uploadName = null;
            qn.textContent = "NO MEDIA QUEUED";
            qn.classList.remove("loaded");
